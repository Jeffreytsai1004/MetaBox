...
def getMayaWin(widPtr=None):
    ...
    return shiboken6.wrapInstance(pointer, QtWidgets.QWidget)
...
class Frameless(
    ...
    def __init__(self, parent=getMayaWin()):
        ...
        # --can generate errors with PySide6
        ...
        if self == obj:
            ...
            if (event.type() == QtCore.QEvent.MouseMove):
                ...
                if event.buttons() == QtCore.Qt.MidButton:
                    ...
                    out = event.globalPos() - self.prev - self.centralWidget().pos()
                    self.window().move(self.prev - self.offset + out)
                ...
                if event.buttons() == QtCore.Qt.RightButton:
                    ...
                    self.window().resize(self.prevSize.width() + out.x(),
                                self.prevSize.height() + out.y())
    ...
    def setDock(self,
        ...
        """
        Obsolete since this class is MayaQWidgetDockableMixin now
        """
    ...
    def addBar(self,
        ...
        map(lambda x: x.setMinimumHeight(self.miniSize - 6),
            (self.butTheme, self.butWindo, self.butAbout))

        map(lambda x: x.setIconSize(QtCore.QSize(50, 50)),
            (self.butTheme, self.butWindo, self.butAbout))

        self.butTheme.setCheckable(True)
        self.butWindo.setCheckable(True)

        self.butTheme.clicked.connect(self.wrapThemeEvent)
    ...
    def restoreTheme(self):
        ...
        themeName = self.settings.load('theme')

        if themeName == "dark":
            self.butTheme.setChecked(True)

        self.themeEvent(themeName)
    ...
    def wrapThemeEvent(self):

        if not self.sender() and not self.objectName():
            return

        themeName = "dark" if self.sender().isChecked() else "light"

        self.settings = Settings(self.objectName())
        self.settings.save('theme', themeName)

        self.themeEvent(themeName)


    def themeEvent(self, themeName):
        """Description
        """

        if not hasattr(self, "butTheme"):
            return
        ...
        factoryStyle = "fusion" if themeName == "dark" else "Oxygen"
        factory = QtWidgets.QStyleFactory.create(factoryStyle)

        if factory:
            self.setStyle(factory)
...
class ziCollapse(QtWidgets.QGroupBox, QtCore.QObject):
    ...
    def __init__(self, url=None, parent=None):
        ...
        self.setStyleSheet(self.getColorStyle())
    ...
    def mouseMoveEvent(self, event):
        ...
        if QtCore.QRect(self.width()*.33, 3.5,
                        self.width()*.33, 6.5).contains(event.pos()):

            self.inside = True
            self.setCursor(QtCore.Qt.ClosedHandCursor)

            if self.open:
                self.setStyleSheet(self.getColorStyle("(255, 255, 255)"))
    ...
    def collapse(self):
        ...
        stSheet = self.neutral if self.open else self.getColorStyle("(126, 131, 90)")
        self.setStyleSheet(stSheet)

    def setNeutral(self, valueSize=14):
        self.neutral = "\nQGroupBox{font-size: %dpx;font-weight: bold;}" % (valueSize)

    def getColorStyle(self, color="(120, 120, 120)"):

        outColor= self.neutral.split("}")
        outColor.append("color: rgb%s;}" % (color))

        output = ";".join(outColor)
